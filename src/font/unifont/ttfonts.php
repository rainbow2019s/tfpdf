...
class TTFontFile
{
    ...
    public function getMetrics($file)
    {
        ...
        $this->descent     = 0;
        $this->TTCFonts    = array();
        $this->version     = $version     = $this->readUlong();
    ...
    public function readTableDirectory()
    {
        ...
        $this->searchRange   = $this->readUshort();
        $this->entrySelector = $this->readUshort();
        $this->rangeShift    = $this->readUshort();
        $this->tables        = array();
        for ($i = 0; $i < $this->numTables; $i++) {
            $record                       = array();
            $record['tag']                = $this->readTag();
            $record['checksum']           = array($this->readUshort(), $this->readUshort());
            $record['offset']             = $this->readUlong();
            $record['length']             = $this->readUlong();
    ...
    public function getTablePos($tag)
    {
    ...
    public function seekTable($tag, $offset_in_table = 0)
    {
        $tpos       = $this->getTablePos($tag);
    ...
    public function getChunk($pos, $length)
    {
        fseek($this->fh, $pos);
        if ($length < 1) {return '';}
    ...
    {
        list($pos, $length) = $this->getTablePos($tag);
...
/////////////////////////////////////////////////////////////////////////////////////////
    ...
    public function extractInfo()
    {
        ...
        $numRecords         = $this->readUshort();
        $string_data_offset = $name_offset + $this->readUshort();
        ...
        for ($i = 0; $i < $numRecords; $i++) {
            ...
            if ($platformId == 3 && $encodingId == 1 && $languageId == 0x409) {
                ...
                $opos       = $this->_pos;
                $N          = $this->getChunk($string_data_offset + $offset, $length);
                $this->_pos = $opos;
                $this->seek($opos);
            }
            if ($N && $names[$nameId] == '') {
        ...
        $this->seekTable("head");
        $this->skip(18);
        $this->unitsPerEm = $unitsPerEm = $this->readUshort();
        ...
        $this->skip(3 * 2);
        $indexToLocFormat = $this->readUshort();
        $glyphDataFormat  = $this->readUshort();
        ...
        if (isset($this->tables["hhea"])) {
            $this->seekTable("hhea");
            $this->skip(4);
            $hheaAscender  = $this->readShort();
            $hheaDescender = $this->readShort();
        ...
        if (isset($this->tables["OS/2"])) {
            ...
            $this->skip(26);
            $sTypoAscender  = $this->readShort();
            $sTypoDescender = $this->readShort();
            if (!$this->ascent) {
        ...
        $this->seekTable("post");
        $this->skip(4);
        $this->italicAngle        = $this->readShort() + $this->readUshort() / 65536.0;
        $this->underlinePosition  = $this->readShort() * $scale;
        $this->underlineThickness = $this->readShort() * $scale;
        $isFixedPitch             = $this->readUlong();
        ...
        $this->seekTable("hhea");
        $this->skip(32);
        $metricDataFormat = $this->readUshort();
        if ($metricDataFormat != 0) {
            die('Unknown horizontal metric data format ' . $metricDataFormat);
        }

        $numberOfHMetrics = $this->readUshort();
        ...
        $this->skip(2);
        $cmapTableCount      = $this->readUshort();
        ...
        for ($i = 0; $i < $cmapTableCount; $i++) {
            ...
            if (($platformID == 3 && $encodingID == 1) || $platformID == 0) {
                // Microsoft, Unicode
                $format = $this->getUshort($cmap_offset + $offset);
                if ($format == 4) {
...
/////////////////////////////////////////////////////////////////////////////////////////
    ...
    public function makeSubset($file, &$subset)
    {
        ...
        $this->skip(32);
        $metricDataFormat = $this->readUshort();
        $orignHmetrics    = $numberOfHMetrics    = $this->readUshort();
        ...
        list($start, $dummy) = $this->getTablePos('glyf');
        ...
        $tags = array('name');
        foreach ($tags as $tag) {$this->add($tag, $this->getTable($tag));}
        $tags = array('cvt ', 'fpgm', 'prep', 'gasp');
        foreach ($tags as $tag) {
            if (isset($this->tables[$tag])) {$this->add($tag, $this->getTable($tag));}
        }

        // post - PostScript
        $opost = $this->getTable('post');
        ...
        // glyf - Glyph data
        list($glyfOffset, $glyfLength) = $this->getTablePos('glyf');
        if ($glyfLength < $this->maxStrLenRead) {
            $glyphData = $this->getTable('glyf');
        }
        ...
        foreach ($subsetglyphs as $originalGlyphIdx => $uni) {
            ...
            if ($glyfLength < $this->maxStrLenRead) {
                ...
                if ($glyphLen > 0) {
                    $data = $this->getChunk($glyfOffset + $glyphPos, $glyphLen);
                } else {
                    $data = '';
                }
    ...
    public function getGlyphs($originalGlyphIdx, &$start, &$glyphSet, &$subsetglyphs)
    {
        ...
        $numberOfContours = $this->readShort();
        if ($numberOfContours < 0) {
            $this->skip(8);
            $flags = GF_MORE;
            while ($flags & GF_MORE) {
                $flags    = $this->readUshort();
                $glyphIdx = $this->readUshort();
    ...
    public function getHMTX($numberOfHMetrics, $numGlyphs, &$glyphToChar, $scale)
    {
        ...
        if (($numberOfHMetrics * 4) < $this->maxStrLenRead) {
            $data = $this->getChunk($start, ($numberOfHMetrics * 4));
            $arr  = unpack("n*", $data);
        ...
        $data = $this->getChunk(($start + $numberOfHMetrics * 4), ($numGlyphs * 2));
        $arr  = unpack("n*", $data);
    ...
    public function getLOCA($indexToLocFormat, $numGlyphs)
    {
        ...
        if ($indexToLocFormat == 0) {
            $data = $this->getChunk($start, ($numGlyphs * 2) + 2);
            $arr  = unpack("n*", $data);
            for ($n = 0; $n <= $numGlyphs; $n++) {
                $this->glyphPos[] = ($arr[$n + 1] * 2);
            }
        } else if ($indexToLocFormat == 1) {
            $data = $this->getChunk($start, ($numGlyphs * 4) + 4);
            $arr  = unpack("N*", $data);
            for ($n = 0; $n <= $numGlyphs; $n++) {
    ...
    public function getCMAP4($unicode_cmap_offset, &$glyphToChar, &$charToGlyph)
    {
        ...
        $this->skip(6);
        $endCount = array();
        for ($i = 0; $i < $segCount; $i++) {$endCount[] = $this->readUshort();}
        $this->skip(2);
        $startCount = array();
        for ($i = 0; $i < $segCount; $i++) {$startCount[] = $this->readUshort();}
        $idDelta = array();
        for ($i = 0; $i < $segCount; $i++) {$idDelta[] = $this->readShort();} // ???? was unsigned short
        $idRangeOffset_start = $this->_pos;
        $idRangeOffset       = array();
        for ($i = 0; $i < $segCount; $i++) {$idRangeOffset[] = $this->readUshort();}
